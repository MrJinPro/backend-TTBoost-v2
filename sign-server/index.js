...
// Optional signer plugin. Not required for the JS LIVE bridge itself.
let signer = null;
try {
  ...
  console.warn('tiktok_signer.js не найден. HTTP /sign будет отвечать 501, но LIVE bridge продолжит работать.');
}
...
app.post('/sign', (req, res) => {
  ...
  if (!signer) {
    return res.status(501).json({
      ...
      hint: 'Добавьте sign-server/tiktok_signer.js с export function signWebcastRequest({ url, userAgent })',
    });
  }
  try {
...
httpServer.listen(PORT, () => {
  console.log(`TTBoost TikTok bridge listening on http://localhost:${PORT}`);
});