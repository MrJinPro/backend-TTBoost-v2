...
Примечания:
- Если получаете `404`, смотрите **тело ответа**:
  - `{"detail":"license not found"}` — ключ не найден в БД (не выдавался/ошибка при вводе).
  - `{"detail":"Not Found"}` (или HTML 404) — эндпоинт/прокси не найден (возможна старая версия backend или неверный nginx proxy).
